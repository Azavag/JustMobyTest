    ...
    public class SpriteColorizer : MonoBehaviourLinkerRuntime<FigmaConverterUnity>
    {
        public async Task ColorizeSprites(List<FObject> fobjects)
        {
            ...
            foreach (FObject fobject in fobjects)
            {
                ...
                {
                    // When using SpriteRenderer, only allow graphics that consist of a single solid _sprite.
                    if (!fobject.Data.Graphic.HasSingleColor)
                        continue;
                }
                else if (monoBeh.IsUITK() || monoBeh.IsNova())
                {
                    // For UITK or Nova, only single-_sprite graphics are allowed.
                    if (!fobject.Data.Graphic.HasSingleColor)
                        continue;

                    // When using UITK or Nova, allow only single-_sprite graphics (no gradients at all).
                    if (fobject.Data.Graphic.HasSingleGradient)
                        continue;
                }
                else
                {
                    // For all other image components, allow either a single _sprite or (conditionally) a single gradient.
