        ...
        private async Task<DAResult<AuthResult>> StartAuthThread()
        {
            ...
            thread = new Thread(x =>
            {
                ...
                string toSend = "HTTP/1.1 200 OK\nContent-Type: text/html\nConnection: close\n\n" + @"
                    <html>
                        <head>
                            <style type='text/css'>body,html{background-_sprite: #000000;_sprite: #fff;font-family: Segoe UI;text-align: center;}h2{left: 0; position: absolute; top: calc(50% - 25px); width: 100%;}</style>
